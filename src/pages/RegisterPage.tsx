...
import { 
  User, Mail, Lock, Eye, EyeOff, ArrowRight, AlertCircle, CheckCircle, X
} from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Если identities пустой, значит юзер уже есть в базе
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-center items-center p-12 overflow-hidden bg-neutral-900 border-r border-white/5">
        ...
        >
            ...
            <motion.p variants={fadeInVariant} className="text-lg text-gray-400 mb-8 leading-relaxed">
                Построй свой MVP без команды, используя AI. Присоединяйся к сообществу создателей будущего.
            </motion.p>
        ...
        >
            <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">
                    Уже есть аккаунт? <a href="/login" className="text-purple-400 hover:text-purple-300 font-bold hover:underline transition-colors">Войти</a>
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <motion.div className="space-y-1.5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Имя Фамилия</label>
                    <div className="relative group">
                        ...
                        <input 
                            type="text" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Ivan Ivanov" required
                ...
                <motion.div className="space-y-1.5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Подтвердите пароль</label>
                    <div className="relative group">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-purple-400 transition-colors" size={18} />
                        <input 
                            type={showPassword ? "text" : "password"} name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="••••••••" required
                ...
                {/* Error Message */}
                {error && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex items-center gap-2 text-red-400 text-sm bg-red-400/10 p-3 rounded-lg border border-red-400/20">
                        <AlertCircle size={16} />
                        <span>{error}</span>
                    </motion.div>
                )}
                ...
                {/* Submit Button */}
                <motion.button 
                    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
                    type="submit" disabled={loading}
                    className={`w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold py-4 rounded-xl shadow-[0_0_20px_rgba(147,51,234,0.3)] transition-all flex items-center justify-center gap-2 group mt-6 ${loading ? 'opacity-70 cursor-wait' : ''}`}
                >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-center text-xs text-gray-600 mt-6">
                Регистрируясь, вы принимаете <a href="#" className="underline hover:text-gray-400">Правила</a> и <a href="#" className="underline hover:text-gray-400">Политику</a>.
            </motion.div>
        </motion.div>
      </div>
    </div>
